...
def main():
    base_model = tf.keras.applications.DenseNet121(input_shape=(64, 64, 3), include_top=False,
                                                   weights='imagenet')
    model = Sequential()
    ...
    # model summary
    model.summary()
    model.save('./model/model-DenseNet121.h5')
    ...
    plot_confusion_matrix(cm, classes=list(map_characters.values()), title='DenseNet121 confusion matrix')
    plt.show()

    # loss
    hist = model.fit(xtrain, ytrain, validation_split=0.3, epochs=20, batch_size=1000)
