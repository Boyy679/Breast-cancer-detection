...
def main():
    base_model = tf.keras.applications.DenseNet201(input_shape=(64, 64, 3), include_top=False,
                                                   weights='imagenet')
    model = Sequential()
    ...
    # model summary
    model.summary()
    model.save('./model/model-DenseNet201.h5')
    print('DenseNet201 model saved')
    ...
    plot_confusion_matrix(cm, classes=list(map_characters.values()), title='DenseNet201 confusion matrix')
    plt.show()
